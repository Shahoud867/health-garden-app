...
const GARDEN_META = [
  { type: 'cactus' as const, labelEn: 'Sugar-free', labelUr: 'شوگر فری', color: '#d96d20' },
  { type: 'sunflower' as const, labelEn: 'Movement', labelUr: 'حرکت', color: '#6c9e36' },
  { type: 'bellflower' as const, labelEn: 'Hydration', labelUr: 'پانی', color: '#3b8f9f' },
  { type: 'bamboo' as const, labelEn: 'Protein', labelUr: 'پروٹین', color: '#dca11b' },
  { type: 'succulent' as const, labelEn: 'Consistency', labelUr: 'تسلسل', color: '#7c7d4b' },
] as const

export default function HomeScreen({ navigate, lang, state }: Props) {
  const { user, today, garden } = state
  const firstName = user.name?.trim().split(/\s+/)[0] || (lang === 'ur' ? 'دوست' : 'there')
  ...
  return (
    <div className="min-h-screen pb-24 pt-3">
      <div className="px-4">
        <div className="flex items-start gap-3">
          <div className="flex-1">
            ...
            <h1 className="mt-1.5 max-w-[8.5ch] text-[clamp(1.35rem,3.3vw,1.9rem)] font-semibold leading-[1] tracking-[-0.06em] text-[#241f15]">
              {lang === 'ur' ? 'ہر اچھا قدم\nآپ کو بڑھاتا ہے۔' : 'Every good choice\ngrows you.'}
            </h1>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                ...
                <p className="mt-0.5 text-[10px] text-[#5f523f]">
                  {lang === 'ur' ? 'آپ بہت اچھا کر رہے ہیں!' : "You're on fire!"}
                </p>
              </div>
              <div className="flex items-center gap-1">
                {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, index) => (
                  <span
                    ...
                    style={{ background: index < streak ? '#d96d20' : '#fff8ee' }}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-[13px] text-[#6e5d4a]">
                {lang === 'ur' ? 'ہر پودا ایک عادت کی نمائندگی کرتا ہے۔' : 'Each plant reflects a habit you are growing.'}
              </p>
            </div>
            <button
              type="button"
              ...
              return (
                <div key={meta.type} className="flex flex-col items-center text-center">
                  ...
                  <div className="flex h-11 items-end justify-center">
                    <PlantImage plant={meta.type} stage={stage} dormant={!plant.metToday} size={44} />
                  </div>
        ...
        <div className="mt-6">
          <h2 className="font-heading text-[22px] font-semibold tracking-[-0.04em] text-[#241f15]">
            {lang === 'ur' ? 'آپ کیا کرنا چاہیں گے؟' : 'What would you like to do?'}
          </h2>

          <div className="mt-3.5 grid gap-2.5 sm:grid-cols-2">
            <ActionTile
              title={lang === 'ur' ? 'کھانا درج کریں' : 'Log a Meal'}
              subtitle={lang === 'ur' ? 'اپنی خوراک ٹریک کریں' : 'Track your nutrition'}
            ...
            <ActionTile
              ...
              subtitle={lang === 'ur' ? 'اپنے جسم کو حرکت دیں' : 'Move your body'}
            ...
            <ActionTile
              ...
              subtitle={lang === 'ur' ? 'ہفتہ وار رجحان دیکھیں' : 'See your trend'}
              onClick={() => navigate('weight')}
              color="#e3ab25"
              icon={<span className="text-xl">⚖️</span>}
            />
        ...
        <Panel tone="warm" className="mt-6 flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[16px] font-extrabold text-[#241f15]">
                {lang === 'ur' ? 'آپ بہت اچھا کر رہے ہیں!' : "You're doing amazing!"} ✨
              </p>
        ...
        {state.user.conditions.includes('diabetes') && (
          <Panel className="mt-6 p-4">
            <SectionLabel>{lang === 'ur' ? 'ذیابیطس کا نرم اشارہ' : 'Diabetes tip'}</SectionLabel>
