...
export default function ProfileScreen({
  ...
  const save = async () => {
    setSaving(true)
    try {
      const heightCm = parseFloat(form.heightCm) || state.user.heightCm
      const weightKg = parseFloat(form.weightKg) || state.user.weightKg
      await updateProfile(authId, {
        full_name: form.name,
        height_cm: heightCm,
        weight_kg: weightKg,
      })
      setState({ user: { ...state.user, name: form.name, heightCm, weightKg } })
      setEditing(false)
    } catch (err) {
      showToast(
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `health-garden-export-${new Date().toISOString().slice(0, 10)}.json`
      a.click()
      URL.revokeObjectURL(url)
      showToast(
        lang === "ur"
          ? "ڈیٹا ڈاؤن لوڈ ہو گیا"
      ...
      <div className="mx-auto max-w-[460px]">
        ...
        <div className="mt-6">
          <Section title={lang === "ur" ? "پروفائل" : "Profile"}>
            {editing ? (
              <div className="space-y-3 py-1">
                {[
                  {
                    label: lang === "ur" ? "نام" : "Name",
                    key: "name",
                    type: "text" as const,
                  },
                  {
                    label: lang === "ur" ? "قد (cm)" : "Height (cm)",
                    key: "heightCm",
                    type: "number" as const,
                  },
                  {
                    label: lang === "ur" ? "وزن (kg)" : "Weight (kg)",
                    key: "weightKg",
                    type: "number" as const,
                  },
                ].map((f) => (
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    onClick={save}
                    disabled={saving}
                    className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[#6c9e36] px-4 py-3 font-extrabold text-white disabled:opacity-70"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 rounded-full border border-[#e6d5ba] bg-[#fff8ee] px-4 py-3 font-extrabold text-[#2c2418]"
                  >
                    {t("cancel", lang)}
                  </button>
                </div>
              ...
              <>
                ...
                />
                <Row
                  label={lang === "ur" ? "عمر" : "Age"}
                  value={`${state.user.age}`}
                />
                <Row
                  label={lang === "ur" ? "قد" : "Height"}
                  value={`${state.user.heightCm} cm`}
                />
                <Row
                  label={lang === "ur" ? "وزن" : "Weight"}
                  value={`${state.user.weightKg} kg`}
                />
                <Row
                  label={lang === "ur" ? "ترمیم" : "Edit profile"}
          ...
          <Section title={t("billing", lang)}>
            {isPremium ? (
              <Row
                label={lang === "ur" ? "فعال پریمیم" : "Premium active"}
                onPress={() => navigate("premium")}
              />
            ) : (
              <Row
                label={lang === "ur" ? "مفت منصوبہ" : "Free plan"}
                onPress={() => navigate("premium")}
              />
            )}
          </Section>
          ...
          <Section title={t("installApp", lang)}>
            <Row
              ...
              value={
                lang === "ur" ? "ہوم اسکرین پر شامل کریں" : "Add to Home Screen"
              }
            />
            <Row
              label="iOS Safari"
          ...
          <Section
            title={lang === "ur" ? "ڈیٹا اور رازداری" : "Data & Privacy"}
          >
            <button
              type="button"
            ...
            >
              ...
              {exporting ? (
                ...
                <span className="text-sm text-[#6e5d4a]">›</span>
              )}
            </button>
            <Row
            ...
            >
              <span className="text-sm font-semibold text-[#d96d20]">
          ...
          <Panel className="w-full max-w-[420px] p-6">
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                disabled={deleting}
                className="flex-1 rounded-full border border-[#e6d5ba] bg-[#fff8ee] px-4 py-3 font-extrabold text-[#2c2418] disabled:opacity-60"
              >
                {t("cancel", lang)}
              </button>
              <button
                ...
                disabled={deleting}
                className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[#d96d20] px-4 py-3 font-extrabold text-white disabled:opacity-70"
              >
