  ...
  return (
    <div className="space-y-5">
      {blocks.map((b, i) => (
        <div key={i}>
          {b.h && (
            <h2 className="font-heading mb-2 text-[17px] font-semibold text-[#241f15]">
              {b.h}
            </h2>
          )}
          {b.p.map((para, j) => (
            <p
              key={j}
              className="mb-2 text-[13.5px] leading-[1.75] text-[#5f523f] last:mb-0"
            >
              {para}
            </p>
          ))}
  ...
  return (
    <div className="min-h-screen px-5 pb-24 pt-5">
      <div className="mx-auto max-w-[460px]">
        ...
        <PageTitle
          eyebrow={lang === "ur" ? eyebrowUr : eyebrowEn}
          title={t(doc, lang)}
        />
        ...
        {lang === "ur" && (
          <Panel tone="warm" className="mt-4 p-4">
            <p className="text-[13px] leading-[1.9] text-[#5f523f]">
              یہ صفحہ فی الحال صرف انگریزی میں دستیاب ہے۔ اردو ترجمہ جلد شامل
              کیا جائے گا۔
            </p>
          </Panel>
        )}
