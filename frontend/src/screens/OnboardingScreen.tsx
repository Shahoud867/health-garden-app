...
const CONDITION_OPTIONS: { val: string; en: string; ur: string }[] = [
  {
    val: "diabetes",
    en: t("conditionDiabetes", "en"),
    ur: t("conditionDiabetes", "ur"),
  },
  { val: "pcos", en: t("conditionPCOS", "en"), ur: t("conditionPCOS", "ur") },
  {
    val: "knee_pain",
    en: t("conditionJoint", "en"),
    ur: t("conditionJoint", "ur"),
  },
...
export default function OnboardingScreen({
  ...
  const toggleCondition = (c: string) => {
    ...
    setForm({
      ...form,
      conditions: existing.includes(c)
      ...
      <div className="mx-auto max-w-[460px]">
        ...
        {step !== "disclaimer" && (
          <div className="mb-5">
            <div className="mb-2 flex items-center justify-between text-xs font-bold uppercase tracking-[0.18em] text-[#8b6f46]">
              <span>
                {lang === "ur" ? "مرحلہ" : "Step"} {stepIndex}{" "}
                {lang === "ur" ? "میں سے" : "of"} {STEPS.length - 1}
              </span>
              <span>{Math.round(progress)}%</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-[#eadcc7]">
              <div
                className="h-full rounded-full bg-[#6c9e36] transition-all duration-500"
        ...
        <Panel className="px-5 py-7">
          ...
          {step === "profile" && (
            <>
              <SectionLabel>
                {lang === "ur" ? "آپ کے بارے میں" : "About you"}
              </SectionLabel>
              <PageTitle
                title={
                  lang === "ur" ? "اپنی بنیاد بتائیں" : "Build your baseline"
                }
                subtitle={
                  lang === "ur"
                    ? "یہ معلومات آپ کے اہداف کو ذاتی بناتی ہیں۔"
                    : "These details help personalize your goals."
                }
              />
          ...
          {step === "activity" && (
            <>
              <SectionLabel>
                {lang === "ur" ? "سرگرمی" : "Activity"}
              </SectionLabel>
              <PageTitle
                title={
                  lang === "ur"
                    ? "ایک معمولی ہفتہ کیسا ہے؟"
                    : "What does a typical week look like?"
                }
                subtitle={
                  lang === "ur"
                    ? "بس قریب ترین انتخاب کریں۔"
              ...
              <div className="mt-6 space-y-3">
                {[
                  {
                    val: "sedentary",
                    en: t("actSedentary", "en"),
                    ur: t("actSedentary", "ur"),
                  },
                  {
                    val: "light",
                    en: t("actLight", "en"),
                  ...
                  optionButton(
                    o.val,
                    lang === "ur" ? o.ur : o.en,
                    form.activityLevel === o.val,
                    () => setForm({ ...form, activityLevel: o.val }),
                  ),
                )}
              </div>
            </>
            ...
            <>
              ...
              <div className="mt-6 space-y-3">
                {[
                  ...
                  {
                    val: "maintain",
                    en: t("goalMaintain", "en"),
                    ur: t("goalMaintain", "ur"),
                  },
                  {
                    val: "build_muscle",
                    en: t("goalStrength", "en"),
                    ur: t("goalStrength", "ur"),
                  },
                  {
                    val: "general_health",
                    en: t("goalGeneral", "en"),
          ...
          {step === "conditions" && (
            <>
              ...
              <PageTitle
                title={
                  lang === "ur"
                    ? "کیا کوئی خاص خیال رکھنا ہے؟"
                    : "Anything we should be mindful of?"
                }
                subtitle={
                  lang === "ur"
              ...
              <div className="mt-6 max-h-[46vh] space-y-3 overflow-y-auto pr-1">
                {CONDITION_OPTIONS.map((o) => {
                  const selected =
                    o.val === "none"
                      ...
                      : form.conditions.includes(o.val)
                  return optionButton(
                    o.val,
                    lang === "ur" ? o.ur : o.en,
                    selected,
                    () => toggleCondition(o.val),
                  )
                })}
                ...
                {form.conditions.includes("other") && (
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, otherCondition: e.target.value })
                    }
                    placeholder={
                      lang === "ur" ? "اپنی حالت لکھیں" : "Type your condition"
                    }
            ...
            <div className="text-center">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="rounded-[24px] border border-[#e6d5ba] bg-[#fffaf1] p-5 text-center">
                  <div className="text-[28px] font-black text-[#6c9e36]">
                    {calorieTarget}
                  </div>
                  <div className="mt-1 text-sm text-[#6e5d4a]">
                    {lang === "ur" ? "روزانہ کیلوریز" : "daily calories"}
                  </div>
                </div>
                <div className="rounded-[24px] border border-[#e6d5ba] bg-[#fffaf1] p-5 text-center">
                  <div className="text-[28px] font-black text-[#3b8f9f]">
                    {proteinTarget}g
                  </div>
                  <div className="mt-1 text-sm text-[#6e5d4a]">
                    {lang === "ur" ? "روزانہ پروٹین" : "daily protein"}
                  </div>
                </div>
              </div>
              {form.name && (
                <p className="mt-5 text-[15px] font-semibold italic text-[#2c2418]">
                  {lang === "ur"
                    ? `${form.name}، آپ کا باغیچہ انتظار کر رہا ہے۔`
                    : `${form.name}, your garden is waiting.`}
                </p>
              )}
            </div>
          )}
          ...
          {step !== "disclaimer" && (
            <button
              onClick={next}
              disabled={
                (step === "activity" && !form.activityLevel) ||
                (step === "goal" && !form.goal)
              }
