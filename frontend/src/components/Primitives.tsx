...
export function Panel({
  ...
  return (
    <div
      className={cn('rounded-[28px] border shadow-[0_14px_34px_rgba(56,34,14,0.055)]', tones, className)}
      style={style}
    >
      {children}
    </div>
    ...
    <div className="flex flex-col items-center text-center">
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} className="absolute inset-0 -rotate-90">
          <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={muted} strokeWidth={stroke} />
          <circle
            cx={size / 2}
            cy={size / 2}
            r={radius}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="text-[24px] font-black leading-none text-[#2c2418]">{label}</div>
          {caption && <div className="mt-1 text-[11px] font-semibold text-[#8b6f46]">{caption}</div>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3.5">
        ...
        <div>
          <div className="text-[15px] font-extrabold leading-tight text-white">{title}</div>
          <div className="mt-1 text-[13px] font-medium text-white/88">{subtitle}</div>
        </div>
      </div>
...
export function IconShell({ children, color }: { children: ReactNode; color: string }) {
  return (
    <span
...
function iconFactory(path: ReactNode, viewBox = '0 0 24 24') {
  return function Icon(props: SVGProps<SVGSVGElement>) {
    return (
      <svg viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" {...props}>
        {path}
      </svg>
    )
  }
...
export const FlameIcon = iconFactory(<path d="M13 3c1.2 2.4.5 4.1-.5 5.4-.9 1.2-1.7 1.9-1.7 3.2 0 1.8 1.4 3 3.2 3 2.4 0 4-1.9 4-4.6 0-3.6-2.7-6.1-5-7Z" />)
